...
namespace RollablePlus
{
    ...
    public sealed class RollableVerification : MonoBehaviour
    {
        ...
        IEnumerator Start()
        {
            ...
            hadMusicMute = PlayerPrefs.HasKey("RollablePlus.MusicMuted"); hadSfxMute = PlayerPrefs.HasKey("RollablePlus.SoundEffectsMuted");
            oldMusicMute = PlayerPrefs.GetInt("RollablePlus.MusicMuted"); oldSfxMute = PlayerPrefs.GetInt("RollablePlus.SoundEffectsMuted");
            oldVolume = PlayerPrefs.GetFloat("RollablePlus.Volume", 1f);
            oldBest = PlayerPrefs.GetInt("RollablePlus.Best", 0); oldMuted = PlayerPrefs.GetInt("RollablePlus.Muted", 0); oldEffects = PlayerPrefs.GetInt("RollablePlus.ReducedEffects", 0);
            ...
            Check(Mathf.Approximately(PlayerPrefs.GetFloat("RollablePlus.Volume"), .37f), "Volume preference is recorded");
        ...
        void Finish()
        {
            ...
            if (hadMusicMute) PlayerPrefs.SetInt("RollablePlus.MusicMuted", oldMusicMute); else PlayerPrefs.DeleteKey("RollablePlus.MusicMuted");
            if (hadSfxMute) PlayerPrefs.SetInt("RollablePlus.SoundEffectsMuted", oldSfxMute); else PlayerPrefs.DeleteKey("RollablePlus.SoundEffectsMuted");
            PlayerPrefs.SetFloat("RollablePlus.Volume", oldVolume); PlayerPrefs.SetInt("RollablePlus.Best", oldBest); PlayerPrefs.SetInt("RollablePlus.Muted", oldMuted); PlayerPrefs.SetInt("RollablePlus.ReducedEffects", oldEffects); PlayerPrefs.Save();
            File.WriteAllText(Path.Combine(output, "verification.txt"), string.Join("\n", checks) + "\n\nRuntime errors:\n" + string.Join("\n", errors));
            Debug.Log("ROLLABLE_VERIFICATION_" + (errors.Count == 0 ? "PASSED" : "FAILED") + " checks=" + checks.Count + " errors=" + errors.Count);
            SessionState.SetBool("Rollable.Verifying", false);
